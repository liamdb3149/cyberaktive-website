    ...
    <Section className="section-mesh relative bg-gradient-to-br from-blue-50/30 via-purple-50/20 to-pink-50/30">
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <BentoGrid className="max-w-7xl mx-auto">
            ...
            <BentoItem key={index} span={4}>
              <RevealOnScroll delay={600 + index * 100}>
                <GlassCard className="h-full text-center group hover:scale-105 transition-all duration-300">
                  <div className="w-12 h-12 bg-primary/20 rounded-xl flex items-center justify-center mb-4 mx-auto group-hover:bg-primary/30 transition-colors">
                    <solution.icon className="w-6 h-6 text-primary" />
                  </div>
                  <h4 className="font-bold text-lg mb-3 text-foreground" data-testid={`text-solution-title-${index}`}>
                    {solution.title}
                  </h4>
                  <p className="text-sm text-muted-foreground mb-4" data-testid={`text-solution-description-${index}`}>
          ...
          {/* Big Benefits - Three Cards */}
          {bigBenefits.map((benefit, index) => (
            <BentoItem key={index} span={4}>
              <RevealOnScroll delay={900 + index * 100}>
                <GlassCard className="text-center group hover:scale-105 transition-all duration-300">
                  <div className="space-y-4">
                    <div className="w-16 h-16 bg-gradient-to-br from-accent/20 to-primary/20 rounded-2xl flex items-center justify-center mx-auto group-hover:scale-110 transition-transform">
          ...
          <BentoItem span={12}>
            <RevealOnScroll delay={1200}>
              <div className="text-center mt-16">
                <GlassCard className="max-w-2xl mx-auto bg-gradient-to-r from-primary/10 via-accent/5 to-primary/10">
                  <h4 className="premium-text-lg text-foreground mb-4 font-bold">
                    This isn't wishful thinking – it's the reality for property managers using AI automation
                  </h4>
                  <p className="text-muted-foreground">
                    Transform your business from reactive firefighting to proactive growth, 
                    with systems that work 24/7 so you don't have to.
                  </p>
                </GlassCard>
              </div>
            </RevealOnScroll>
