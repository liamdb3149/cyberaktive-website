...
import { ArrowRight, Zap, Calendar, Mail, CheckCircle } from "lucide-react";
import Header from "@/components/layout/header";
import Footer from "@/components/layout/footer";
import { RevealOnScroll } from "@/components/ui/visual";
import { Button } from "@/components/ui/button";
import saasWorkflowPages from "@/data/saasWorkflowPages";

const BOOKING_URL = "https://api.leadconnectorhq.com/widget/booking/tlZAXVaHxTw9fvistaTr";

export default function SaasWorkflowsHub() {
  useEffect(() => {
    document.title = "AI Workflow Automation for SaaS Teams | Cyberaktive";

    const metaDesc = document.querySelector('meta[name="description"]');
    if (metaDesc) metaDesc.setAttribute("content", "Custom AI workflow automation for SaaS and tech companies. From code review to customer onboarding - we build the workflows that eliminate your team's biggest time sinks. 90-day ROI guarantee.");

    const ogTitle = document.querySelector('meta[property="og:title"]');
    if (ogTitle) ogTitle.setAttribute("content", "AI Workflow Automation for SaaS Teams | Cyberaktive");

    const canonicalHref = "https://cyberaktive.com/saas/workflows/";
            ...
            <RevealOnScroll>
              <div className="max-w-3xl">
                ...
                <h1 className="text-4xl md:text-5xl lg:text-6xl font-extrabold text-white leading-[1.1] mb-6 tracking-tight">
                  AI Workflow Automation for SaaS Teams
                </h1>

                <p className="text-lg text-gray-400 mb-8 leading-relaxed max-w-2xl">
                  We build custom AI workflows that eliminate the repetitive, high-cost work slowing down your engineering, sales, support, and ops teams. Every workflow is delivered as a working system you own outright - no subscriptions, no lock-in.
                </p>
            ...
            <RevealOnScroll>
              <div className="max-w-2xl mb-12">
                ...
                <p className="text-gray-500">
                  10 proven automation categories, each targeting the highest-ROI bottleneck for SaaS and tech companies.
                </p>
              </div>
            </RevealOnScroll>

            <div className="grid grid-cols-1 md:grid-cols-2 gap-5 max-w-4xl">
              {saasWorkflowPages.map((workflow, i) => (
                <RevealOnScroll key={i} delay={i * 60}>
                  <Link href={`/saas/${workflow.slug}`}>
                    <div className="group bg-white border border-gray-200 rounded-xl p-6 hover:border-blue-300 hover:shadow-md transition-all duration-200 cursor-pointer h-full">
                      ...
                      <h3 className="text-base font-bold text-gray-900 mb-2">{workflow.workflowName}</h3>
                      <p className="text-sm text-gray-500 mb-4 leading-relaxed">
                        {workflow.intro.split('.')[0]}.
                      </p>
                ...
                <div className="grid md:grid-cols-2 gap-12 items-center">
                  <div>
                    <h2 className="text-3xl md:text-4xl font-bold text-white mb-6 leading-tight">
                      Ready to Cut Your Team's Busywork?
                    </h2>
